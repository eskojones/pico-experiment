...
void term_display (Surface *surface, TermInfo *term) {
    ...
    for (int x = 0, y = 0, i = 0, l = term->width * term->height; i < l; i++) {
        if (term->output[i] != 0) {
            tmp[0] = term->output[i];
            font_print(surface, tmp, x * term->font_width, 1 + y * term->font_height, term->colours[i]);
        }
        if (++x >= term->width) {
            x = 0;
            y++;
    ...
    font_print(surface, term->cursor, term->cursor_x * term->font_width, 1 + term->cursor_y * term->font_height, term->cursor_colour);
}
...
int main () {
    ...
    lcd_init();
    lcd_set_backlight(1024);
    Surface *screen = surface_create(LCD_WIDTH, LCD_HEIGHT);
    TermInfo *term = term_create(LCD_WIDTH / 8, LCD_HEIGHT / 8);
